## Mix + Kad Get Example (No Gateway)
## Demonstrates anonymous `getValue` through mix without a gateway.
## The exit mix node dials the Kad node directly.
...
proc mixKadGetNoGatewaySimulation() {.async: (raises: [Exception]).} =
  ...
  let getConn = senderMix
    .toConnection(
      MixDestination.init(destKad.switch.peerInfo.peerId, destKad.switch.peerInfo.addrs[0]),
      KadCodec,
  ...
  info "Anonymous kad get succeeded (no gateway)",
    key = key,
    discoveredValue = string.fromBytes(discoveredValue)

when isMainModule:
  waitFor(mixKadGetNoGatewaySimulation())
